...
{
    printf("\nLoading Material: %s %s", filename.c_str(), name.c_str());
...
void EngineSimApplication::renderScene()
{
    ...
    if (m_camera.rotation.x > 2.0f * ysMath::Constants::TWO_PI)
        m_camera.rotation.x -= ysMath::Constants::TWO_PI;
    if (m_camera.rotation.x < -2.0f * ysMath::Constants::TWO_PI)
        m_camera.rotation.x += ysMath::Constants::TWO_PI;
